...
#accumulates the sum of the minimum values of cubes
sum = 0

for i in 0..input.split.length - 1 do
    #find the word 'Game', reset the counters
    if input.split[i].match(/Game/)
        #checks the value of each multiplication in every game
        mul = 0 

        #start of the game, searching for minimum values of each color
        red = 0
        green = 0
        blue = 0
	end

    #finding for each color
	if input.split[i].match(/red/) && input.split[i - 1].to_i > red
        red = input.split[i - 1].to_i
        puts "red #{input.split[i - 1]}"
	end

	if input.split[i].match(/green/) && input.split[i - 1].to_i > green
        green = input.split[i - 1].to_i
        puts "green #{input.split[i - 1]}"
	end

	if input.split[i].match(/blue/) && input.split[i - 1].to_i > blue
        blue = input.split[i - 1].to_i
        puts "blue #{input.split[i - 1]}"
	end

    #checking the multiplication
    if mul < red * green * blue
        
        #always remove first, the first entry starts with 0 so no prob
        sum = sum - mul

        #now u can substitute to the actual mul to be compared
        mul = red * green * blue
        sum = sum + mul
