...
#which game we analysing
game = 0

#accumulates the sum of the valid games
sum = 0

#flag that verifies if that game is still couting
valid = 1
...
for i in 0..input.split.length - 1 do
	...
	if input.split[i].match(/red/)
        if flag == 1 && input.split[i - 1].to_i > 12
            sum = sum - game
            flag = 0
        end
	end

	if input.split[i].match(/green/) 
        if flag == 1 && input.split[i - 1].to_i > 13
            sum = sum - game
            flag = 0
        end
	end

	if input.split[i].match(/blue/) 
        if flag == 1 && input.split[i - 1].to_i > 14
            sum = sum - game
            flag = 0
        end
	end
